	...
	public function main($id, $mode)
	{
		...
		switch ($action)
		{
				...
				if ($mode == 'acp')
				{
					...
					if ($this->request->is_set_post('submit') && sizeof($add_styles))
					{
						...
						if (sizeof($duplicate_styles))
						{
							...
							{
								$duplicate_message .= (empty($duplicate_message) ? '' : '<br>') . $this->user->lang('ERROR_DUPLICATE_STYLE_PATH', $this->ext_root_path . 'app/styles/' . $path) . ' | ' . $this->user->lang('ERROR_DUPLICATE_STYLE_NAME', $name);
							}
						}

						$success_lang = $this->user->lang('STYLE_DATA_ADDED');
						$error_lang = $this->user->lang('ERROR_DUPLICATE_STYLE', $duplicate_message);
			...
			case 'edit':
				...
				if ($this->request->is_set_post('submit'))
				{
					...
					if (empty($style_name))
					{
						trigger_error($this->user->lang('ERROR_STYLE_NAME_EMPTY'). adm_back_link($this->u_action), E_USER_WARNING);
					}
					else
					{
						...
						if (sizeof($rows))
						{
							trigger_error($this->user->lang('ERROR_STYLE_NAME_EXIST', $style_name). adm_back_link($this->u_action), E_USER_WARNING);
						}
					}
					...
					trigger_error($this->user->lang('STYLE_DATA_UPDATED') . adm_back_link($this->u_action));
				}
			break;
			...
			case 'delete':
				if ($mode == 'acp')
				{
					...
					{
						confirm_box(false, $this->user->lang['CONFIRM_DELETE_STYLE_DATA'], build_hidden_fields(array(
							'i'			=> $id,
							'mode'		=> $mode,
							'id'		=> $style_id,
							'action'	=> 'delete',
		...
		if ($mode == 'acp')
		{
			...
			$this->template->assign_vars(array(
				...
				'ADD_STYLE_EXPLAIN'		=> sizeof($style_dirs) ? $this->user->lang('ACP_ADD_ACP_STYLE_EXPLAIN') : $this->user->lang('ACP_ADD_ACP_STYLE_UNAVAILABLE', $this->ext_root_path . 'app/styles/'),
			));
		}
	}
